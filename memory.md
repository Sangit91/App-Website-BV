...
0. **Đặc tả hệ thống AI Agents `ai-agents-spec.md` (2026-08-03):** user yêu cầu tài liệu mô tả hệ thống AI agents (OpenCode + OpenBrain + AGENTS.md/agents/ + memory/) để tái tạo trên dự án khác. Khảo sát thực tế: `opencode.json` (plugin `./openbrain`, provider 9router), plugin OpenBrain clone từ `github.com/CodebyKDvn/openbrain` (3 tools: search_memories/list_skills/create_skill; hooks tool.execute.after ghi episodic, system.transform chèn context, session.idle ghi reflection), DB `~/.opencode/openbrain/openbrain.db` (ngoài git). Viết file `ai-agents-spec.md` (root, cạnh dactaupdate.md) — 9 mục: tổng quan 4 lớp, OpenCode config, OpenBrain plugin, AGENTS.md/agents/ (9 file), memory 3 tầng, spec docx, workflow session chuẩn, pitfalls (7.x), checklist tái tạo, định nghĩa thành công.

0. **Sửa chuỗi "Sở Y tế tỉnh Quảng Nam" + tên viết tắt header (2026-08-03):** user xác nhận bệnh viện trực thuộc **Sở Y tế Thành phố Đà Nẵng** (không phải tỉnh Quảng Nam). Sửa: (1) letterhead `NewsDetailModal.tsx` — "SỞ Y TẾ TỈNH QUẢNG NAM"→"SỞ Y TẾ THÀNH PHỐ ĐÀ NẴNG", "BVĐK KV MIỀN NÚI PHÍA BẮC"→"BỆNH VIỆN ĐA KHOA KHU VỰC MIỀN NÚI PHÍA BẮC QUẢNG NAM"; (2) nội dung tin "phẫu thuật nội soi khớp gối" trong `src/data.js` + `src/data.ts` + `server/scripts/seed-news.ts` — "Sở Y tế tỉnh Quảng Nam"→"Sở Y tế Thành phố Đà Nẵng" + UPDATE DB `news` (1 row news-1); (3) `ai.service.ts` SYSTEM_INSTRUCTION địa chỉ "Huyện Đại Lộc, tỉnh Quảng Nam"→"107 Quang Trung, Xã Đại Lộc, TP. Đà Nẵng"; (4) `.env.example` VITE_APP_NAME="BVĐK KV Miền Núi Phía Bắc Quảng Nam" + VITE_HOSPITAL_ADDRESS="107 Quang Trung, Xã Đại Lộc, TP. Đà Nẵng" + VITE_HOTLINE="02353.747.433". Giữ nguyên 3 thầu legacy còn lại (mô tả phạm vi đấu thầu cấp địa phương — dữ liệu thật). Verify: grep sạch, lint+build pass, bvdh-frontend + bvdh-backend healthy.

0. **Đồng bộ CTA Gọi Hotline + Đặt lịch vào modal hướng dẫn `/cho-benh-nhan` (2026-08-03):** `OutpatientGuideModal.tsx` + `InpatientGuideModal.tsx` áp dụng cùng format footer như DetailModal/SpecialtyModal: "Đóng" (secondary) + **"Gọi Hotline: {DEFAULT_CONTACT.hotline}"** — anchor `tel:` (`hotline.replace(/\./g,"")`) class `bg-brand-green hover:bg-green-700 text-white` + **"Đặt lịch khám"** (`window.open("/dat-kham","_blank")`, giữ "In hướng dẫn" riêng của Inpatient). Import `Phone` + `DEFAULT_CONTACT` từ `siteContact`; gỡ icon thừa (`X`,`Clock`). Lưu ý: Button UI không có variant `outline` → dùng anchor class thuần thay vì Button variant. Verify: lint + build pass, bvdh-frontend healthy.

14. **🔴 Lưu ý PHI tra cứu bệnh sử là WEB DEMO (2026-08-03):** Frontend tra cứu bệnh sử (`PatientLookupForm.tsx` + `PatientPortalSection.tsx`) hiện **chạy MOCK** — `PatientLookupForm.handleSubmit` tự dựng bệnh nhân giả (`Nguyễn Văn Minh`, `id:'mock-patient-id'`, KHÔNG gọi `/api/v1/patients/lookup`), và 3 fetch PHI trong `PatientPortalSection.handlePatientLookup` **không gắn Bearer/readToken** (sẽ 401/403 vì backend đã khoá). Toàn bộ `src/` không có file gọi `otp/send`/`otp/verify`/`readToken`. **ĐÂY LÀ CHỦ ĐÍCH (intended):** hệ thống chỉ là webdemo; luồng liên thông HIS thật (OTP → readToken → Bearer fetch PHI + consent) đã được đặc tả ghi sẵn trong spec docx section **Patient Portal HIS** + `agents/06-server-api.md` (HIS API Standards). **Khi GO LIVE, dev sẽ tự dựng lại toàn bộ frontend này theo đặc tả** — không phải bug cần sửa ở phase demo.

0. **Search thầu `/thong-tin-thau` — fix layout + jitter (2026-08-03):** theo review UX: (1) nút "Tìm" từng gây vỡ input → đưa vào TRONG pill `overflow-hidden`, bố cục **icon ở bên PHẢI cố định, input + nút Tìm mở rộng sang TRÁI** (viền phải neo chặt → icon không dời → hết jitter; trước đây icon trái + nút Tìm ngoài khiến wrapper phình làm icon dời 22px → hover flapping). (2) Placeholder rút gọn "Nhập tiêu đề hoặc ngày đăng (dd/mm/yyyy)". (3) Badge "Chính xác & Công khai" KHÔI PHỤC lại header (bên trái search pill, `hidden sm:flex`) — trước đó bị bỏ theo review UX #4 nhưng user yêu cầu giữ. (4) **Fix gốc hover không bật:** pill thêm `relative z-50` + `justify-end` — trước đó nút float AI (`fixed bottom-6 right-6 z-40`) CHE phủ icon search khi scroll 0 (elementsFromPoint xác nhận) → hover/click vô hiệu; thêm `relative z-50` nâng search lên trên float, `justify-end` neo icon vào viền phải pill (trước đó flex content 75px tràn làm icon nằm ngoài pill 30px). (5) **Kích thước:** pill thu gọn `h-10 w-10` (40px, tăng từ 30px), placeholder `placeholder:text-xs` (12px, nhỏ hơn text nhập 14px). Verify: badge hiện, 40px → hover 320px hovered=true ổn định sau 1s, icon x=1377 trong viewport, search "báo giá" 3 kết quả. lint+build pass, restart.

0. **Search thầu `/thong-tin-thau` thu gọn icon (2026-08-03):** thanh search đổi thành pill icon kính lúp (thu gọn `w-[30px] h-[30px]`), hover/focus-within animate mở rộng `hover:w-80` (320px) để nhập. State `isSearchOpen` cho touch/mobile (click icon mở + focus input), `searchInputRef`. **Bug fix:** pill dùng `group-hover:w-80` nhưng chính nó là `.group` → không tự hover; đổi thành `hover:w-80` trên pill, children mới dùng `group-hover:opacity-100`. clearSearch đóng pill. **Bố cục:** search pill nằm TRONG header row (cùng div `mb-6 flex items-center justify-between` với h2 + badge "Chính xác & Công khai", căn phải); **chiều cao pill `h-[30px]` bằng đúng badge** (py-1.5 + text-xs + border); badge "N kết quả" nằm dưới header. Verify: 30px = 30px, hover 320px, search "báo giá" 3 kết quả. lint+build pass, restart.

0. **Search thầu `/thong-tin-thau` (2026-08-03):** thêm ô tìm kiếm + nút "Tìm kiếm" (Enter cũng chạy). `matchesSearch(item, query)`: match theo tiêu đề/summary (contains) HOẶC theo ngày cụ thể `dd/mm/yyyy` (VD 15/07/2026) hoặc tháng `mm/yyyy` (VD 07/2026) — chuẩn hoá separator `/ . -`, đọc token từ `date` + `formatDateShort(tenderStartDate/End)` + `publishedAt` (vi-VN). **Bug đã fix:** destructuring month sai `const [, qm, qy]` trên mảng 2 phần tử → `qm="2026"` → fix `const [qm, qy]`. Kết quả filter trên `deptTenders` (theo phòng ban), reset page=1, hiện badge "N kết quả cho ..." + nút xoá, empty state riêng khi không tìm thấy. Verify Playwright: "07/2026"→1, "15/07/2026"→1, "báo giá"→3. lint+build pass, bvdh-frontend restart.

0. **Paging thầu `/thong-tin-thau` 6/trang (2026-08-03):** trước đây `deptTenders` (lọc theo phòng ban) render toàn bộ (~24 card). Thêm `currentPage` state + `PAGE_SIZE=6`, slice theo trang, nút phân trang (prev/number/next) hiện khi `totalPages>1`; reset page=1 khi đổi phòng ban (trong onClick nút dept). Xoá `featuredTender` dead. Backend đã sắp `publishedAt desc` nên 6 tin đầu là mới nhất. Verify Playwright: 6 card/trang, 4 trang cho CNTT, sang trang 2 OK. lint+build pass, bvdh-frontend restart.

0. **Nâng cấp `DetailModal` (cơ sở vật chất `/gioi-thieu#co-so-vat-chat`) (2026-08-03):** áp dụng cùng format CTA như dịch vụ/chuyên khoa: footer 3 nút — "Đóng" outline secondary + **"Gọi Hotline: {hotline}"** (`tel:` từ `getSection("contact")`) + **"Đặt lịch khám"** (dispatch `bvdk:open-booking`). Gọn bố cục: `max-w-2xl`→`max-w-xl`, padding `p-6 md:p-10`→`p-5 md:p-7`, `space-y-6`→`space-y-4`, ảnh `h-56`→`aspect-[16/9]`, thêm box hướng dẫn liên hệ (Calendar icon). Component public dùng chung (chỉ GioiThieuPage dùng — 2 chỗ facility + value đều nâng cấp, không ảnh hưởng admin tabs vốn dùng modal riêng). Verify Playwright `/gioi-thieu#co-so-vat-chat`: modal mở + CTA tel:02353747432 + Đặt lịch + Đóng + box hướng dẫn. lint+build pass, restart.

0. **Đồng bộ format modal chuyên khoa `/chuyen-khoa` (2026-08-03):** trước đây `SPECIALTY_CONTENT` chỉ có 7/19 khoa (tim/sản/nhi/cấp cứu/ngoại/CĐHA/TMH/RHM) → các khoa còn lại (Nội tiết, Thận nhân tạo, Cơ Xương Khớp, Da Liễu, Tâm lý, Thẩm mỹ, Dược, Mắt, Xét nghiệm, Y tế dự phòng, Ung bướu, ICU, Nội chung) mở modal thiếu services/experts → format lệch. **Fix:** bổ sung 13 entry mới vào `SPECIALTY_CONTENT` (`noi-chung/noi-tiet/than-nhan-tao/co-xuong-khop/da-lieu/tam-ly/tham-my/duoc/mat/xet-nghiem/yte-du-phong/ung-buou` + ICU map `cap-cuu`) + mở rộng `specialtyContentKey` (thứ tự ưu tiên: nội chung→nội tiết→hồi sức/icu→ung bướu→thận→xương khớp→da liễu→tâm lý→thẩm mỹ→xét nghiệm→y tế dự phòng→dược→mắt→tim→sản→nhi→ngoại→hình ảnh→mũi/tai→răng). Thêm icon Eye/Pill/Microscope/Shield vào import SpecialtyModal. Verify Playwright 7 khoa đại diện: services+experts+info+cta đều true. lint+build pass, restart. (Ghi chú: verify script đầu flaky do đóng modal chưa kịp — chờ modal đóng hẳn là pass.)

0. **Fix modal chi tiết chuyên khoa `/chuyen-khoa` (2026-08-03):** theo review UX: (1) **Thiếu CTA** → footer 3 nút: "Đóng" outline secondary + **"Gọi Hotline: {hotline}"** (`tel:` từ `getSection("contact")`) + **"Đặt lịch khám"** (dispatch `bvdk:open-booking` mở BookingForm). (2) **Bổ sung thông tin:** "Kỹ thuật & dịch vụ mũi nhọn" (chips services) + "Đội ngũ chuyên gia" — lấy từ `SPECIALTY_CONTENT` (đã `export` từ `SpecialtyModal.tsx`), map theo tên khoa qua helper `specialtyContentKey` (tim→tim-mach, sản→san-khoa, nhi→nhi-khoa, ngoại→ngoai-tong-hop, hình ảnh→chan-doan-hinh-anh, mũi/tai→tai-mui-hong, răng→rang-ham-mat). (3) **Info strip** 3 ô: Địa điểm (Quầy tiếp nhận - Nhà A) + Giờ làm việc (T2-T7 07:00-16:30) + Hotline. (4) **Cân bằng khoảng trắng:** `max-w-2xl`→`max-w-xl`, padding gọn, ảnh `aspect-[16/9]`. Verify Playwright `/chuyen-khoa` (tab Nội tổng quát → Khoa Tim mạch): header + services chips + experts + info strip + CTA tel:02353747432 + Đặt lịch + Đóng đều hiện. lint+build pass, restart.

0. **Fix modal chi tiết dịch vụ `/dich-vu` (2026-08-03):** theo review UX: (2.1) ảnh "Kiến thức thai sản" từng dùng `sanphukhoa-1.jpeg` (bị chê là cảnh thiên nhiên/hẻm núi) → đổi `s2`+`s4` trong `siteServices.ts` sang `hero-thaisan.jpeg` (hero Sản & Nhi, chủ đề mẹ-bé). (2.2) Gọn chiều cao modal: `max-w-2xl`→`max-w-xl`, padding `p-6 md:p-10`→`p-5 md:p-7`, `space-y-6`→`space-y-4`, ảnh `h-56` cố định → `aspect-[16/9]` auto. (2.3) Footer: "Đóng" thành outline secondary + **thêm CTA "Gọi Hotline: {hotline}"** primary xanh (`tel:` từ `getSection("contact")`, import `DEFAULT_CONTACT`); header giữ nguyên dark-green "Chi tiết dịch vụ". Verify Playwright `/dich-vu`: header, ảnh hero-thaisan, badge giá, CTA tel:02353747432, Đóng, box hướng dẫn đều hiện. lint+build pass, restart.

0. **Fix modal thầu `/thong-tin-thau` dùng chung NewsDetailModal (2026-08-03):** `ThongTinThauPage` trước đây có modal inline riêng (gradient xanh, không theo đặc tả). Đã thay bằng `<NewsDetailModal>` dùng chung (như trang chủ + `/tin-tuc`), xoá `handleDownload`/`downloadToast` + modal inline (~100 dòng). Cải tiến `NewsDetailModal` theo báo cáo layout: (1) countdown badge tách label + giá trị (`text-peach` label + `whitespace-nowrap` số), tránh nền đè chữ; (2) **nút "Tải Hồ Sơ Thầu" luôn bật dù hết hạn** (chỉ hiện badge "Thầu đã kết thúc" làm thông tin — vẫn cho tải hồ sơ để xem/kiểm tra khi đoàn kiểm tra yêu cầu, fix 2026-08-03); (3) tên file PDF `truncate whitespace-nowrap min-w-0` giữ 1 dòng, không làm vỡ nút tải. lint + build pass, verify Playwright: `/thong-tin-thau` + `/tin-tuc` mở chung modal Cổng Công Bố + In Lưu Trữ; thầu hết hạn hiện badge kết thúc + nút tải disabled. bvdh-frontend restart.

0. **Fix modal tin tức chuẩn "Công bố văn bản" dùng chung (2026-08-03):** trước đây có 2 modal: `News.tsx` (đã chuẩn A/B/C Cổng Công Bố) và `TinTucPage.tsx` (modal đơn giản cũ "Tin tức y khoa" — thiếu In Lưu Trữ/Quốc hiệu/Metadata/Chữ ký/Chia sẻ). Refactor: tách `src/components/public/NewsDetailModal.tsx` (modal chuẩn, tự chứa countdown thầu + print CSS + download toast) + `src/lib/tender.ts` (helpers parseTenderDate/getTenderStatus/formatTenderDate/getTimeLeft/getStatusBadge/isTenderPost). Cả `News.tsx` và `TinTucPage.tsx` đều dùng `NewsDetailModal` — xoá modal inline trùng lặp. lint + build pass, verify Playwright `/tin-tuc`: header Cổng Công Bố + In Lưu Trữ + Quốc hiệu + Metadata + Ký + Share + Đóng TL đều hiện. bvdh-frontend restart.

0. **Fix navbar "Tra cứu bệnh sử online" (2026-08-03):** mega menu (desktop + mobile) Navbar có nhánh `link.includes("bệnh sử") || link.includes("Tra cứu") -> onOpenAI()` khiến click link này mở trợ lý AI thay vì tới trang tra cứu. Đã bỏ nhánh AI để link fall-through điều hướng đúng `/cho-benh-nhan#cong-thong-tin` (PatientPortalSection). lint + build pass, bvdh-frontend restart.

0. **Spec v3.2 (02/08/2026)** — đã đặc `Dac-ta-Master-v3_2-SRS-TRD.docx` & rà soát hệ thống. Đã chốt: **giữ Admin login overlay dark** (chỉ thêm 2FA, không chuyển split-screen 3.6.2 — chờ ban lãnh đạo). Plan hoàn thiện theo Wave S/C/O ghi tại `plan-production.md` §6 (2FA, error-code, upload security, CMS workflow, RBAC dept, session timeout, password policy, cache+SEO, ESLint, backup, NFR). ~10 mục thiếu còn lại, ưu tiên 2FA + API error + upload security.
