...
- Ghi Changelog tóm tắt KHỐI 1.5: "v3.1 — OTP flow thay token minting, refresh rotation + reuse detection, CCCD encryption, Zod validation toàn bộ route, pagination thật, per-form rate limit bucket, health check + graceful shutdown, audit logging middleware, RBAC enforce helpers, activity_logs index + PHI audit fields, password hash clarification, rate limit matrix, logo thật, admin login redesign, specialty images, admin tabs mở rộng, CMS nội dung tĩnh site_content, Tender tab DB-backed".
