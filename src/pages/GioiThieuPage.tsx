  ...
  return (
    <Layout>
      ...
      <section ref={contentRef} id="ve-chung-toi" className="py-24">
        <div className="max-w-[1580px] mx-auto px-4 xl:px-8 2xl:px-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {[
              { icon: Building2, label: "Phòng mổ", value: "5 phòng mổ", color: "bg-peach/20 text-peach" },
              { icon: Users, label: "Trình độ", value: "100% sau ĐH", color: "bg-brand-green/10 text-brand-green" },
              { icon: Activity, label: "Chuyên khoa", value: "10+ chuyên khoa", color: "bg-blue-50 text-blue-600" }
            ].map((item, idx) => (
              <motion.div
                key={item.label}
                initial={{ opacity: 0, y: 30 }}
      ...
      <DetailModal
        ...
        headerIcon={Heart}
        headerLabel="Giá trị cốt lõi"
        title={selectedValue?.title}
        image={selectedValue?.image}
        description={selectedValue?.desc}
      />
    </Layout>
  );
}